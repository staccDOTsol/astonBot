...
const binance = require('node-binance-api')().options({
  APIKEY: 'cQxZeaewqZuXcj4rZ2dk6RSKAZY6yc15sruRZuWpQd8Z0L39cPaFBUyynay1f8AR',
  APISECRET: 'flDJ0q1s7xErrxH3Fcg967VEZiInHpJ1kSZ64Ei0bFCv1JBM6pHeqtZ42do0Lr6i',
...
if ( position == 0){
	...
	if (enternewtps){
		...
		console.log('new tps...')
		takeprofits = []
		thetp = entryPrice * (1- 1-.0025)
		await client.createOrder(  'BTC/USDT', "Limit", 'buy', position / 2, entryPrice * (1 - 0.0025))
		takeprofits.push(await client.createOrder(  'BTC/USDT', "Limit", 'buy', position / 2, entryPrice * (1 - 0.006)))
	}
}
if (first){
...
if (last >= theSMA[theSMA.length-2] * (1+(0.005)) && buying && theRSI[theRSI.length-1].k > 80){
	buying = false;
...
thetp = last * (1 - 0.0025)
	...
	await client.createOrder(  'BTC/USDT', "Limit", 'sell', amt, last - 100)
	takeprofits = []
		await client.createOrder(  'BTC/USDT', "Limit", 'buy', amt / 2, last * (1 - 0.0025))
		takeprofits.push(await client.createOrder(  'BTC/USDT', "Limit", 'buy', amt / 2, last * (1 - 0.006)))
...
if (tps[t] != 0 && tps[t] < last){
	...
	sl= [];
	sls= []
		await client.createOrder(  'BTC/USDT', "Limit", 'buy', position, last + 100)
}
}
for (var s in sl){
if (sl[s] != 0 && sl[s] < last){
...
if (orderb[o] != 0 && orderb[o] * (1+(0.005)) < last && buying2 && !buying){
	...
	tps= []
		...
		enternewtps = true;
	console.log(new Date() + ' safety sell x1! Price: ' + last + ' & prev SMA: ' + theSMA[theSMA.length-2])
		await client.createOrder(  'BTC/USDT', "Limit", 'sell', amt, last - 100)
...
if (orderb[o] != 0 && orderb[o] * (1+(0.015)) < last && buying3 && !buying){
	...
	cancelall()
	enternewtps = true
	console.log(new Date() + ' safety sell x2! Price: ' + last + ' & prev SMA: ' + theSMA[theSMA.length-2])
		await client.createOrder(  'BTC/USDT', "Limit", 'sell', amt, last - 100)
}
}
if (first){
